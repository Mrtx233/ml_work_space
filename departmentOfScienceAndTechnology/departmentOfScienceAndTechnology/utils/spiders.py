...
class BasePortiaSpider(CrawlSpider):
    ...
    def parse_list(self, response ,current_page=1, repeat_dup_pages=0,use_custom_pagination=False, **kwargs):
        """
        新增列表页解析
        允许为空
        """
        for sample in getattr(self, 'list_items', []):
            for definition in sample:
                ...
                for result in self.load_item(definition, response):
                    ...
                    if not total_page_matched and result.get("total_page"):
                        total_page = result["total_page"]
                        total_page_matched = True
                ...
                self.logger.info(
                    f"[----------] 当前页爬取任务数：{task_num},  当前页总任务数：{len(combined)}"
                )
                self.logger.info(
                    f"[----------] 当前页：{current_page}, 总页数：{total_page}"
                )
                # 统计连续重复页
                if task_num == 0:
                    repeat_dup_pages += 1
                ...
                if repeat_dup_pages >= 3:
                    self.logger.info(f"[----------] 连续{repeat_dup_pages}页均为重复数据，停止翻页")
                    return
                # 下一页
                if next_page and not use_custom_pagination:
                    self.logger.info(f"[----------] 翻页 -> {next_page}")
                    yield Request(next_page, callback=self.parse_list, cb_kwargs={'current_page': current_page + 1, 'repeat_dup_pages': repeat_dup_pages})
                elif use_custom_pagination and kwargs.get('base_url') and kwargs.get('make_url_name'):
                    # JS分页处理
                    make_url_method = getattr(self, kwargs['make_url_name'])
                    max_page = None
                    if total_page:
                        try:
                            max_page = int(total_page)
                        except (ValueError, TypeError):
                            self.logger.warning(f"无法解析总页数: {total_page}")
                    if max_page and current_page >= max_page:
                        self.logger.info(f"[LIST] 已达到最大页数 {max_page}，停止翻页")
                        return
                    next_page_url = make_url_method(current_page, kwargs['base_url'])
                    self.logger.info(f"[LIST] 自定义翻页 -> {next_page_url}")
                    yield Request(
                        next_page_url,
                        callback=self.parse_list,
                        cb_kwargs={
                            'current_page': current_page + 1,
                            'repeat_dup_pages': repeat_dup_pages,
                            'use_custom_pagination': True,
                            'base_url': kwargs['base_url'],
                            'make_url_name': kwargs['make_url_name']
                        }
                    )
