...
_IMAGE_PATH_RE = re.compile(r'/?(?:[^/]+/)*(?:.+\.(?:mng|pct|bmp|gif|jpg|jpeg|png|pst|psp|tif|tiff|ai|drw|dxf|eps|ps|svg))')
...
class BaseProcessor(object):
    ...
    def __str__(self):
        ...
        joined = reversed(list(izip_longest(reversed(args), reversed(defaults), fillvalue=_SENTINEL)))
...
class Field(BaseProcessor):
    def __init__(self, name, selector, processors=None, required=False, type='css', file_category=''):
...
class Text(BaseProcessor):
    def __call__(self, values):
        return [remove_tags(v).strip() if v and isinstance(v, six.string_types) else v for v in values]
...
class Number(BaseProcessor):
    def __call__(self, values):
        ...
        for value in values:
            ...
            txt = _NUMERIC_ENTITIES.sub(lambda m: six.unichr(int(m.groups()[0])), value)
...
class Url(Text):
    def __call__(self, values, loader_context=None):
        ...
        for value in values:
            ...
            base = loader_context.get('baseurl', '') if loader_context else ''
...
class Image(Text):
    def __call__(self, values):
        return super(Image, self).__call__([
            val if isinstance(val, (dict, list)) else extract_image_url(val) for val in values])
...
class Regex(BaseProcessor):
    ...
    def __call__(self, values):
        ...
        for value in values:
            ...
            if not match:
                continue
            results.append(u"".join([g for g in match.groups() or match.group() if g]))
        return results
