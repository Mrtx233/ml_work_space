...
def bind_live_update_to_text(t: tk.Text):
    t.bind("<KeyRelease>", lambda e: update_preview(), add=True)
    t.bind("<FocusOut>", lambda e: update_preview(), add=True)

def bind_live_update_to_entry(ent: tk.Entry):
    ent.bind("<KeyRelease>", lambda e: update_preview(), add=True)
    ent.bind("<FocusOut>", lambda e: update_preview(), add=True)
...
def update_preview():
    ...
    raw_url = normalize_user_url(url_entry.get())
    cn_name = normalize_cn_name(cn_entry.get())

    domain = ""
    if raw_url:
        domain = urlparse(raw_url).netloc.strip("/")

    base = domain_to_filename(domain) if domain else ""
    target_normal = ""
    target_gk = ""
    if spiders_dir and cn_name and base:
        target_normal = os.path.join(spiders_dir, cn_name, f"{base}.py")
        target_gk = os.path.join(spiders_dir, cn_name, f"{base}_gk.py")
    ...
    try:
        ...
        preview = []
        preview.append("# ========= 输出预览 =========")
        preview.append(f"spiders_dir = {repr(spiders_dir)}")
        preview.append(f"中文目录名 = {repr(cn_name)}")
        preview.append(f"URL = {repr(raw_url)}")
        preview.append(f"域名 = {repr(domain)}")
        preview.append("")
        preview.append(f"普通 spider 文件 = {repr(target_normal)}")
        preview.append(f"gk spider 文件 = {repr(target_gk)}")
        preview.append("")
        preview.append("# ========= 配置预览 =========")
...
def build_field_row(parent, field_name, default_xpath, widget_dict, is_gov=False):
    ...
    preset_combo = ttk.Combobox(bottom, values=preset_values, state="readonly", width=18)
    preset_combo.current(0)
    preset_combo.grid(row=0, column=2, sticky="w", padx=(0, 8))
...
canvas.bind("<Enter>", _bind_wheel, add=True)
canvas.bind("<Leave>", _unbind_wheel, add=True)
...
preview_box = ttk.LabelFrame(right, text="配置预览（只读）", style="IDE.TLabelframe")
